...
# draw is seeded, and consecutive runs on one machine are bit-identical -- but
# the two LAPACKs disagree on identical seeded inputs. So `make reproduce` here
# reports roughly two thousand moved values: third and fourth significant
# figures of fitted scores, and larger moves in the solver conditioning sweep,
# which exists to probe where float64 breaks down and is therefore the most
# sensitive thing in the study. That is a different LAPACK, not a failure to
# reproduce. `make results` and the analyses themselves run correctly here; it
# is the bit-comparison against macOS-generated numbers that cannot pass. See
# docs/testing.md.
