...
- **Coverage is a ratchet.** `fail_under` in `pyproject.toml` sits just under the figure CI measures, which is the one taken without any of the four datasets and about half a point below what a full checkout reports. Measure that basis before you touch the floor: a fresh `git worktree` reproduces it exactly. Raise it when coverage genuinely rises; never lower it to make a run pass. If it trips, find out what stopped running, and check whether something is simply untested before concluding that tests cannot reach it.
