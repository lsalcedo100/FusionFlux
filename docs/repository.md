...
├── ishcdb.py                        # the stellarator-heliotron database: fetch, pin, documented columns, frozen plan
...
├── analysis_ishcdb.py               # the second locked replication, on a database that shares no device with HDB5
...
- `ishcdb.py` owns the International Stellarator-Heliotron Confinement Database, version 26, which is the stellarator counterpart of the ITPA database and shares no device with it. It is public, so unlike CICLOP it is fetched on demand and pinned by SHA-256 like the other deposits. Its columns are documented, so the lock names them and there is no hand-written mapping: the module drops the rows the documentation calls predictive (W7-X and ITER), takes the standard set as the database flags it, follows the standard set's own target convention, and converts watts and per-cubic-metre densities to the units ISS04 is written in. It goes through `ciclop.verify_frozen_plan`, the same guard as the first replication, and not a copy of it.
- `analysis_ishcdb.py` owns the second replication. Both splits, the matched scoring, the verdict rule and the distance diagnostic are `analysis_ciclop`'s, called and not copied, which is what makes the two replications one experiment run twice. What is its own is ISS04 as the reference outside the ranking, and `centred_errors`, which removes each device's mean log residual: ISS04 could not fit these devices with one constant, so a power law is expected to miss a held-out stellarator by a constant factor, and this separates that from wrong trends.
