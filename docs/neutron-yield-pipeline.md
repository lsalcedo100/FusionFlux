...
**Nothing in this document supports a scientific claim.** This pipeline predicts `neutron_yield` from plasma operating conditions, and the dataset it ships with is *synthetic*: generated by `neutron_yield.features.create_synthetic_dataset` from a hand-crafted signal, so any accuracy number it produces measures how learnable that generator is and nothing else.
...
- train the candidate regressors from `neutron_yield/training_registry.py` (`baseline` median dummy, `random_forest`, `hist_gradient_boosting`) on a log-transformed target
...
- When `--allow-synthetic` is used, synthetic data generation still assumes the sampling ranges in `neutron_yield.features.create_synthetic_dataset`, fixed six-row shot blocks, and a hand-crafted yield signal based on Lawson ratio, input energy, pressure, the IPB98 proxy, fuel purity, and Gaussian noise.
