...
extrapolator. Two controls bound that. Regressing
the stored energy, which leaves the loss power among the features and moves only
the identity $\tau = W/P$, leaves the forest worse on \textbf{9 of 13} labels
and 9 of 11 devices at a mean gap of $+0.220$; in dimensionless coordinates,
where no feature is a device label, it is 11 of 13 and 9 of 11. On the
confinement time and the engineering features it is 13 of 13 and 11 of 11. It
survives nested tuning, threshold and fold sweeps, and equal label weighting.
The forest's error tracks the held-out label's Mahalanobis distance from the
training data at $\rho=+0.85$, against $-0.06$ for the power law. At a cut
...
component, which is this work's best cross-validated model and does not reverse
by label or by device. Extrapolation failure tracks long-range saturation rather
than flexibility.
\end{abstract}
...
% This declaration is a top-level section of its own, deliberately, rather than a
% block inside Acknowledgments. IOP requires it, it names nobody, and it must
% survive anonymisation; Acknowledgments does not. Kept separate, the anonymous
% build strips one and keeps the other with no lifting, and there is no marker
% pair for a later edit to move text across.
