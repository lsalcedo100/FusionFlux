...
IPB98(y,2). Held out one device at a time that ordering reverses: the best
cross-validated of the three is its worst extrapolator, so where every flexible
candidate saturates, cross-validation selects against the property the law is
bought for. The candidate set fails, not the score: this work's best
cross-validated model, a linear-plus-RBF Gaussian process, does not reverse.
Two controls bound the inversion, regressing the stored energy instead and
regressing in dimensionless coordinates where no feature is a device label,
which leave the forest worse on 9 of 13 and 11 of 13 labels against
\textbf{13 of 13} on the engineering features. The interval is the sharper failure. At a cut reproducing ITER's $1.82\times$ jump in
major radius, nominal 90\% split-conformal intervals calibrated on held-out
discharges cover \textbf{3\%} and \textbf{0\%} of held-out rows at a width that
moves by under 1.5\%. That invariance is forced: a
split-conformal half-width is a rank statistic of the calibration residuals and
cannot respond to the test distribution, so the interval carries no signal that
it has stopped covering, and its width is no evidence that it survived the
boundary. The forest's error does track the held-out label's Mahalanobis
distance, at $\rho=+0.85$ against $-0.06$ for the power law, so the warning the
interval withholds is available in advance. Two repairs
help, both supplying long-range structure rather than capacity: Connor--Taylor
constraints, whose Kadomtsev rung is free in sample, and that same process.
Holding nonlinear capacity fixed and varying only the long-range mean isolates
the mechanism: saturation, not flexibility.
\end{abstract}
