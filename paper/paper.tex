...
0.15~\cite{hdb5}, which is exactly the long-range structure
...
The per-label scores behind that correlation are in the Supplementary Material.
...
file carries the areal elongation and nothing that can be converted to the other
without the geometry the deposit omits. No cleaning of these rows can therefore
...
The Supplementary Material scores leave-one-label-out at 10, 20, 30 and 50 rows. Only which labels are \emph{scored} changes: every label stays in every
...
of kernel rather than a tuning parameter. The Supplementary Material fits three
...
$\rho=-0.01$. It is also the only flexible model tested whose \emph{native}
posterior intervals reach nominal coverage out of distribution without conformal
recalibration, covering 92.5\% of held-out rows at nominal 90\% where
split-conformal intervals give the forest 3\% and the booster none. The
constrained power law of Sec.~\ref{sec:dimensional} holds near nominal there
too, at 91\%, but through an externally calibrated interval rather than one the
model supplies itself.
...
The constant-mean row reproduces the squared-exponential rung of the Supplementary Material to six decimals, which is the control: the decomposition is the same model rewritten.
