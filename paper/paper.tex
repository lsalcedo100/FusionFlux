...
(v0.4.5; the DOI for all versions is
...
forward for the same scenario~\cite{doyle07}; ITER has since been rebaselined,
on a schedule and a staging this paper does not attempt to
summarise~\cite{loarte}, so this row is conditional on the operating point
...
distribution. The distance itself needs no held-out device, and it is the one
quantity that is known in advance for a machine that does not exist. The
\emph{correlation} between distance and error is not free in the same way. It is
measured over held-out units, so it arrives with item 1 rather than instead of
it; and distance on its own predicts how hard a unit is rather than which model
will fail on it, since the mean baseline of Sec.~\ref{sec:mechanisms} correlates
with it at $+0.56$ while fitting nothing. What separates the models here is the
contrast, $\rho=+0.85$ for the random forest against $\rho=-0.06$ for the power
law.
...
cuts of the size sweep. Where it does not show is the leave-one-out arms, on
which it gains 0.004 by label and 0.001 by device against that same fit, inside
the resolution Sec.~\ref{sec:limitations} states; the constraint's measurable
benefit is confined to the size sweep, which
Sec.~\ref{sec:limitations} also notes is one machine above the matched cut. The collisionless rung gave the lowest error of any
...
saturation and physics constraints in confinement scaling}, version v0.4.5,
Zenodo (2026).
