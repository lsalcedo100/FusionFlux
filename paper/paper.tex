...
intact at 10 of 13 and 9 of 11 with the margin much reduced. The forest's error tracks the held-out label's Mahalanobis distance from the
training distribution ($\rho=+0.85$) where the power law's does not, and it
cannot predict outside its training range. At a cut reproducing ITER's $1.82\times$ jump in major radius the trees score
...
sample and beats the unconstrained fit at every well-powered size cut, and a
...
forest's being capped by its training range rather than an estimate.
\end{abstract}
...
what the convention prescribes, and on this database that score picks the model
that extrapolates worst of the three. A practitioner following current practice
is therefore not left with an open question about extrapolation. They are led to
the worst available answer, and the more carefully they follow the convention
the more confidently they are led there. Degradation is expected and can be
budgeted for; a selection rule anti-correlated with the goal it is used for
cannot.
...
considerably, so they are the obvious second reference. Two things have to be
said before any number, because they bound what the comparison can mean. Each
law was fitted to a different selection out of DB5.2.3, and each specifies an
elongation this database does not supply under that definition. The delivered
file carries one elongation column, \texttt{KAPPAA}, the areal elongation
$\kappa_a = S/(\pi a^2)$ computed from the plasma cross-sectional area, which is
the definition IPB98(y,2) was fitted to and is the column loaded here. ITPA20
and ITPA20-IL are specified on the separatrix elongation at the geometric axis,
$\kappa$, and converting between the two needs the boundary shape rather than
the area alone, which the delivered columns do not carry. No cleaning of these rows can therefore
score ITPA20 on the quantity it was fitted to, and \emph{no absolute level
reported here is a fair test of either law}. None is offered as one.

What survives that is the relative comparison this paper is about, which does
not depend on either law being scored on its own terms. Evaluated analytically
on the rows analysed here, ITPA20 scores 0.265 over all rows, 0.267 per label
and \textbf{0.272} across the ITER-size-matched cut; ITPA20-IL scores 0.393,
0.446 and 0.363. At that cut ITPA20 sits against the random forest's 0.938 and
the gradient booster's 1.072, factors of 3.4 and 3.9, and essentially level with
the fold-fitted power law's 0.278.
...
This is the selection argument of Sec.~\ref{sec:intro}, now measured: the
convention does not merely fail to inform about extrapolation, it picks the
worst of the three extrapolators on offer.
...
laws outside fusion, reported in Secs.~S4 and~S5 of the Supplementary Material,
reproduces the
...
The comparison is also narrower than the question it answers. Table~\ref{tab:reversal}
sets a pooled global fit against ensembles and concludes for the pooled fit,
having not tested the estimator the clustered-validation literature itself
recommends for a new-cluster target: a mixed model with device-level random
intercepts, predicting an unseen device from the population-level fixed effects.
That is the deployment-matched estimator, it is neither a power law nor a tree,
and it is not scored here. What the results above establish is that the ordering
of these models reverses, not that a pooled power law is the best available
predictor of an unseen device.
...
The prose is bound to those artifacts in the same way. Eighty-nine of the
numbers quoted above are tied by test to the field of the generated file they
came from, in both directions: if an analysis is rerun and its output moves, the
