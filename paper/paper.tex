...
Archived at \href{https://doi.org/10.5281/zenodo.22971392}{doi:10.5281/zenodo.22971392}
(v0.4.15; the DOI for all versions is
\href{https://doi.org/10.5281/zenodo.22215142}{10.5281/zenodo.22215142})
\end{center}

\begin{abstract}
\noindent
Confinement scalings fitted across existing tokamaks are used to size
machines not yet built, so validation must measure performance on an unseen
device. On the
ITPA Global H-mode Confinement Database (HDB5, STD5; 6228 slices, 18 labels),
cross-validation grouped by discharge scores a random forest
29\% better in log error than a power law refitted in the same folds and 36\%
better than published IPB98(y,2). Holding out one device at a time reverses
the order of the three primary models: both tree ensembles that win
cross-validation extrapolate worst, and the forest loses to that power law on
% ishcdb:begin
\textbf{13 of 13} eligible labels, and on \textbf{7 of 7} eligible devices
of a stellarator-heliotron database locked before its data were seen.
% ishcdb:end
Regressing the stored energy, or in
dimensionless coordinates carrying less device identity, leaves the forest worse on
9 of 13 and 11 of 13 labels. At a cut inside
the database reproducing ITER's $1.82\times$ jump in major radius, nominal
90\% split-conformal intervals calibrated on held-out discharges cover
\textbf{3\%} and \textbf{0\%} of held-out rows for both ensembles at a width
moving under 1.5\%. A split-conformal half-width is
a rank statistic of the calibration residuals and cannot warn that it has
stopped covering. The forest's error can: it tracks the held-out label's
Mahalanobis distance from the training data at $\rho=+0.85$, against $-0.06$
for the power law. Widening the candidate set repairs the rule: a
linear-plus-RBF Gaussian process, the best cross-validated model, keeps
trending outside the data and does not reverse.
At the size cut, that process and Connor--Taylor constraints on the
exponents, whose Kadomtsev rung costs nothing in sample, both improve
extrapolation. Varying only the long-range mean with the nonlinear part fixed
supports saturation, not flexibility alone, as the mechanism in the models
tested.
\end{abstract}

\begin{center}\small
\textbf{Keywords:} tokamak energy confinement; confinement scaling; machine
learning; extrapolation; cross-validation; uncertainty quantification; ITER
\end{center}
...
headline number in this paper is scored on the nine engineering features, and
the exceptions are labelled where they appear: the ten-column matrix of
Sec.~S14 of the Supplementary Material, which adds IPB98(y,2)'s own prediction
to them and is reported there as a control, and the controls of Sec.~\ref{sec:robustness}
that delete a feature, change the target or move to dimensionless coordinates
...
at stake there; but what they do report, that adding parameters to the power
law beyond a low order improves the in-sample fit and worsens extrapolation
under machine holdout, is the same phenomenon inside the power-law family, and
it is read here as such rather than as a separate finding. So the
contribution is neither the first report of a reversal nor the first
machine-level holdout on this database. The increment is that the held-out
unit is the device and the candidates are flexible models, so the inversion is
between model families rather than between orders of one form; that the
calibrated interval is measured at the same time and fails without any signal
in its width; and that flexibility and long-range saturation are separated,
which says which property to avoid rather than which model. None of these works is the target of
...
\emph{of those three} loses to a log-linear power law on every eligible held-out
database label and every eligible physical device. That qualifier is
load-bearing, and it bounds
...
% ishcdb:begin
The same comparison is then run, under rules fixed and archived before the data
were downloaded, on the stellarator-heliotron confinement
database~\cite{iss04}, which was assembled by a different collaboration and
shares no device with the tokamak one; Sec.~S17 of the Supplementary Material
reports it, and the inversion reproduces there on every eligible held-out
device.
% ishcdb:end
...
so all 6228 delivered rows are analysed. Ref.~\cite{hdb5} describes STD5 as
7537 points from 18 machines; the file the OSF deposit delivers under that name
has 6228, and it is analysed as delivered, so the difference is in what the
deposit supplies and not in any selection applied here. The target is the thermal confinement
...
Sec.~S3 of the Supplementary Material. None of the models in
...
Sec.~\ref{sec:dimensional} and the damping of the bounded correction
(Sec.~S9 of the Supplementary Material) were both
...
one of the two exact collinearities audited in Sec.~S14 of the Supplementary
Material. Wall variants
...
\paragraph{The design matrix.} It is audited in Sec.~S14 of the Supplementary
Material. Standardised, the ten-column matrix that adds the IPB98(y,2)
prediction to the nine features has rank 8, rank deficient by two, and one of
the two dependencies is that a published scaling law added as a model feature
contributes nothing a log-linear fit can use; the nine-column refit of
Eq.~\eqref{eq:powerlaw} drops both redundant columns and has condition number
10.7. Refitting that equation on all rows gives $I_p$ 1.08 against the
published 0.93 and $R$ 1.58 against 1.97, and 77\% of the disagreement lies in
the single weakest singular direction of the design, which carries 0.3\% of its
variance: plasma current traded against machine size, which tokamaks are not
built to vary independently. Why the size dependence in this database is weaker
than IPB98(y,2)'s is an active question that Hall \emph{et al.}~\cite{hall,hall26}
attack directly on DB5.2.3, and Sec.~S14 says how it bears on the size-ordered
cut of Sec.~\ref{sec:size}.

% ishcdb:begin
\paragraph{External replication.} One external replication was specified
before any of its scores were seen. Its rules, the population, the target
convention, the feature mapping, the eligibility threshold and a five-way
verdict rule that includes ``cannot be evaluated'' and ``contradicts the
original result'', were committed to the repository and archived before the
data file was downloaded, and the analysis was run once. The dataset is the
International Stellarator-Heliotron Confinement Database, the stellarator
counterpart of the ITPA one and the database the ISS04 scaling~\cite{iss04}
was fitted to; its primary analysis is that database's own standard set, on
measured rows only, with the target and the regressors mapped as Sec.~S17 of
the Supplementary Material documents. A stellarator has no plasma current, so
the regressors are ISS04's rather than the nine above, and the comparison
between the two databases is of the pattern and not of any number.
% ishcdb:end
...
fixed mean, and the covariance it uses is singular by the rank audit of Sec.~S14
of the Supplementary Material and so
...
considerably. Both are written on the engineering variables of
Eq.~\eqref{eq:powerlaw} with a triangularity factor added, and the elongation
in both is the areal elongation $\kappa_a = S/(\pi a^2)$, the variable
IPB98(y,2) uses and the one the delivered file carries as
\texttt{KAPPAA}~\cite{hdb5}. Each law is therefore evaluated on the column it
was fitted to, which is what makes the three comparable. The full DB5.2.3
revision this paper already pins also carries the boundary elongation
$\kappa$, and the two columns are not interchangeable: substituting the
boundary column for the areal one moves ITPA20 by up to 0.018 and ITPA20-IL by
up to 0.022, all of it for the worse, and enough to reorder ITPA20 against
IPB98(y,2) per label. Sec.~S13 of the Supplementary Material measures that
cost and says why a shape model is the wrong way to find it.
...
ITPA20-IL scores 0.226, 0.247 and \textbf{0.158}, against IPB98(y,2)'s 0.199,
...
gradient booster's 1.072 there ITPA20 is better by factors of 5.7 and 6.5. The
...
under grouped cross-validation a tree can split on those coordinates and fit a
per-device offset: a lookup table, worth a great deal in sample and nothing on
a machine that is not in it.

Deleting those features is the obvious control, and it cannot settle the
question. Refitting on the four
...
5 of 11 devices, at mean gaps of $-0.033$ and $-0.018$. That is not decisive,
because the deleted columns carry the size dependence, which is the thing the
power law extrapolates through. The control removes the suspected leak and
the physics under test together, and the power law's held-out score more than
doubles, from 0.214 to 0.477: the reversal needs features carrying size, and
this cannot say whether because they carry size or because they name the
machine.

The dimensionless coordinates separate exactly those two things, and they are
not coordinates constructed here. Fitting this database in them is established
practice on the regression side as well as the experimental one: Cordey
\emph{et al.}~\cite{cordey} fit the confinement time against $\beta$ and
collisionality directly, and the reference description of the revision analysed
here~\cite{hdb5} reports that transforming an engineering-variable scaling into
dimensionless form leaves large error bars on the result, proposes regression in
the space of dimensionless variables as the alternative, and carries it out.
What this arm adds is the use they are put to: a control on the device-label
pathway, scored under the held-out-device split against the same models. In
...
over the same 6214 rows (the 29\% quoted elsewhere is 29.5\% on all 6228,
rounded), and loses on \textbf{11 of 13} labels and \textbf{9 of 11} devices, at mean gaps
...
is why the forecast of Sec.~S3 of the Supplementary Material is stated in
engineering variables. This arm is a different experiment carrying a different confound
rather than a stricter version of the same one. The specification is in
\texttt{analysis\_dimensionless.py}.

\paragraph{Choices that do not carry it.} Further arms vary choices the
...
10; the five labels below the threshold stay in every training fold either way,
and dropping them from training as well leaves the ordering over the 13
held-out labels unchanged. Repeating the cross-validation over ten shuffled fold assignments puts the
forest's interpolation margin between 27.0\% and 28.5\%, with the deterministic
partition's 29.5\%, the unrounded value of the 29\% quoted throughout, just
above that range, so the margin it later gives up is not
...
$P_{\mathrm{LTH}}$ is one of the nine features. A log-linear model carries the
$-1$ coefficient on $\log P$ exactly and outside the training range of $P$,
because the relation is definitional; a tree approximates it by axis-aligned
splits and, by the bound of Sec.~\ref{sec:mechanisms}, cannot extrapolate it at
all. That admits a reading in which the power law transfers because an identity
is exactly log-linear rather than because it has better long-range structure.
Refitting every model on the eight remaining features tests it: the direction
survives and the strength does not. The forest still wins cross-validation, by
a wider margin, 0.198 against 0.316 where it was 0.128 against 0.181, and
still loses under leave-one-label-out, but on \textbf{10 of 13} labels rather
than 13 and \textbf{9 of 11} devices rather than 11; the exact tests move to
$p=0.092$ and $p=0.065$, from values small only under the idealised null of
Sec.~\ref{sec:data}, the mean paired gap falls from $+0.251$ to $+0.051$,
C-Mod, JT-60U and JET-ILW change hands, and the gradient booster's mean gap
turns negative. Under both feature sets the model that wins the interpolation
comparison is the one that loses the extrapolation comparison, which is the
claim Sec.~\ref{sec:reversal} makes.

\paragraph{The same objection, answered the right way round.} Deleting $P$
answers more than the objection asks, since $P$ is a real predictor as well as
the target's denominator. The control the objection calls for regresses
$\log W_{\mathrm{th}}$ on the same nine features instead, so that $P$ stays as
an ordinary predictor and only the identity goes. STD5 carries no stored
energy, but the full DB5.2.3 revision, pinned by SHA-256 and analysed in
Sec.~S2 of the Supplementary Material, carries \texttt{WTH}, and joining it to
STD5 on machine, shot and time recovers one for \textbf{6214} of the 6228
analysed rows; \texttt{KAPPAA} is delivered in both files and agrees across the
join to $5\times10^{-10}$, and on the joined rows
$W_{\mathrm{th}}/(\tau_{\mathrm{th}}P)$ has a median of 1.000000. Retargeted,
the direction and most of the magnitude survive and the unanimity does not. The
forest still wins cross-validation, 0.123 against the power law's 0.193. Under
leave-one-label-out it loses on \textbf{9 of 13} labels at a mean gap of
$+0.220$, against 13 of 13 and $+0.251$ on $\tau_{\mathrm{th}}$; under
leave-one-device-out it loses on \textbf{9 of 11} at a mean gap of
\textbf{$+0.348$}, larger than the $+0.315$ the confinement time gives; and
...
move to $p=0.267$ and $p=0.065$. This is the stronger control because the design
matrix is unchanged and one coefficient moves: the fitted exponent on $\log P$
goes from -0.688 to $+0.308$, one unit, the identity carried on the other side
rather than taken away, so the slope a tree has to reproduce in that direction
more than halves, the tree's task gets easier, and the gap widens anyway. The specification is in \texttt{analysis\_stored\_energy.py} and
\texttt{results/stored\_energy.json}.

Together the two controls separate what the identity carries from what it does
not: it carries the unanimity, since under either control the count falls from
thirteen to nine or ten and neither exact test then reaches $p=0.05$, and it
does not carry the effect, since deleting only the identity leaves $+0.220$ by
label and $+0.348$ by device and the size cut where it was. Two results this
paper leans on do not weaken. On a held-out label at nominal 90\% the
eight-feature conformal intervals of Sec.~\ref{sec:conformal} cover 42\% for
the forest and 48\% for the booster against 77\% for the power law, where the
nine-feature figures are 35\%, 45\% and 83\%; and in the kernel comparison of
Sec.~\ref{sec:gp} the bounded squared-exponential rung remains the worst of the
three processes, at 0.496 against 0.400 for the linear rung and 0.409 for
their sum, where with $P$ it was 0.541 against 0.214 and 0.218, with the
linear-kernel process still reproducing ridge to four decimals in both arms.
The relation itself is physics rather than a defect in the feature set: the
Connor--Taylor constraint on the exponent of $P$ in Sec.~\ref{sec:dimensional}
is derived from $\tau_E = W/P$, a similarity transformation treating $P$ as an
independent scale would produce a constraint no published law satisfies, and
the constrained rungs therefore cannot be scored under this ablation at all.
...
\paragraph{Rows the standard analysis set excludes.} The same OSF project
publishes the full DB5.2.3 revision, 14153 rows against STD5's 6228, pinned by
its own SHA-256; STD5 is an ELMy-H-mode quality selection out of it, leaving
5358 H-mode rows STD5 does not contain, across 12 labels with zero row
overlap, and 3860 ohmic and L-mode rows in a different confinement regime,
scored against ITER89-P~\cite{iter89p} because an H-mode law is the wrong
baseline for an L-mode plasma. The ranking inverts in both arms and survives
dropping the 336 discharges the disjoint H-mode arm shares with STD5 at
different time slices: against the power law refitted inside each fold, the
like-for-like baseline, a tree ensemble loses on an unseen label in
\textbf{16 of 24} label--model pairs and \textbf{9 of 10}, and against the
published law in 19 of 24 and 10 of 10. Sec.~S2 of the Supplementary Material
reports both arms with the checks on the row matcher. Neither arm is an
independent database: both come from the same ITPA collection and the same
devices, and the five-machine arm is too small to carry a claim alone.

% ishcdb:begin
\paragraph{A database with no device in common.} The stellarator-heliotron
replication of Sec.~S17 of the Supplementary Material was locked before its data
were downloaded, and the outcome was to be reported under a predeclared rule
whichever way it fell. On the standard set, 1719
rows over 7 devices scored at the same
30-row threshold as above, with both splits scored per device
and averaged, the random forest is better than the log-linear power law under
cross-validation, 0.209 against
0.288, and worse on
\textbf{7 of
7} eligible held-out devices,
0.814 against 0.442,
a paired gap of $+0.373$
$[+0.106,
+0.797]$, and degrades
2.54 times as much
between the splits. The booster is worse on
7 of
7. Under the
rule fixed in advance that is a reproduction of the inversion, on devices HDB5
does not contain, and the Gaussian process is again best under both splits and
again does not reverse. Two things differ. The power law's error rises with
distance there, at $\rho=+0.57$ against
the forest's $\rho=+0.64$, where on HDB5 it
is flat; the supplement traces that to the device-by-device constant ISS04
itself needs, which is what the power law is missing on a held-out stellarator
and all it is missing, since with that constant granted its error is unchanged
between the splits and the forest's is not. And on every measured row, a
secondary arm that adds the rows the database's curators excluded, the two
models' holdout errors sit together and the arm is degradation without a
literal inversion.
% ishcdb:end
...
A bounded correction learned on the power law's log residuals and damped takes
it from 0.278 to \textbf{0.206} across this cut, where an unbounded polynomial
correction takes it to 0.356. It is built, scored and qualified in Sec.~S9 of
the Supplementary Material, which also records that cross-validation selects
the full correction in both families while on a held-out label the rung that
transfers best is the uncorrected one; the controlled version of the
comparison, holding the nonlinear component fixed and varying only the
long-range mean, is Sec.~\ref{sec:meanfunction}.
...
experiment: the solver is the one written by hand for the refit of Sec.~S14 of
...
groups are fixed: one row of $C$ when $\rho^{*}$, $\beta$ and $\nu^{*}$ are all
held fixed (Kadomtsev~\cite{kadomtsev}), two when only $\rho^{*}$ and $\beta$
are (collisionless), and three when $\rho^{*}$ alone is (electrostatic), since
fixing fewer groups admits more transformations and therefore imposes more
constraints. Table~S9 of the Supplementary Material sets the hierarchy out;
$C$ and $d$ are built by \texttt{dimensional.constraint\_matrix} from
...
0.15~\cite{ipb98,hdb5}. Neither is fitting this database in the groups of
Eq.~\eqref{eq:groups}, which Sec.~\ref{sec:robustness} attributes where it
uses them: Cordey \emph{et al.}~\cite{cordey} fit it against $\beta$ and
collisionality directly, and the reference description of this
revision~\cite{hdb5} proposes and performs the regression in dimensionless
variables. Its exponents accordingly land on the Kadomtsev
...
ITPA20, published          & \textbf{0.0147} & 0.089 & 0.745 & \textbf{0.165} \\
ITPA20-IL, published       & \textbf{0.0142} & 0.106 & 0.853 & \textbf{0.158} \\
\bottomrule
\end{tabular}
\end{table}
...
Kadomtsev surface than IPB98(y,2) and scores 0.165 against 0.194 at the cut, and
...
bounded hybrid (Sec.~S9) & n/a & 0.151 & 0.246 & 0.246 & 5/11 & 0.206 \\
...
law reported elsewhere gives 0.2778, and Sec.~S14 of the Supplementary Material
shows the choice between them carries nothing. $^{\dagger}$This row's count is that same pair
...
two of them are lower: ITPA20-IL scores 0.158 there and ITPA20 0.165
...
bounded hybrid of Sec.~S9 of the Supplementary Material, which was built specifically for that
...
would fall; as with the bounded correction on a held-out label, cross-validation
does not select it. Every
...
It matters that this is a constraint and not a prior: supplying the same
physics as a Gaussian prior instead, shrunk along the weak singular direction of
the refit, is worth much less, and Sec.~S15 of the Supplementary Material gives
that comparison.
...
scored here, whose two flexible members both saturate, the convention does not
merely fail to inform about extrapolation: it picks the worst of the three
extrapolators on offer, and the more carefully it is followed the more confidently it does so.
...
% ishcdb:begin
The replication on the stellarator database bears on the first of those claims
and not on the second. It says the inversion is not a property of one
compilation, of the standard set's selection criteria or of the machines the
ITPA database happens to hold, since it appears, under a rule written before
the data were seen, on a database with no device in common. It does not
establish a rate across confinement databases or across architectures: it is
one more database and the same three models. And it leaves the mechanism
claim where the Gaussian-process experiment put it, since that experiment was
not run there, though the one model in the candidate set that keeps trending is
again the one that does not reverse.
% ishcdb:end
...
per device on the same rows and features. Where the two disagree, as here by a
factor of 3.6, the disagreement is the result, and the interpolation number
should not be quoted alone.
\item Report how far the target operating point sits from the training
distribution, the one quantity known in advance for a machine that does not
exist. Distance alone says how hard a unit is rather than which model fails on
it, since the mean baseline of Sec.~\ref{sec:mechanisms} correlates with it at
$+0.56$ while fitting nothing; what separates the models is the contrast,
$\rho=+0.85$ for the random forest against $\rho=-0.06$ for the power law.
\item Say whether the model's predictions stop tracking the features outside
the training support, and separately whether they can leave the range of the
training targets at all; both are checkable before anything is fitted. The
second is sharper, since a random forest provably cannot leave that range, but
here it binds on one held-out label of thirteen and across the size cut while
the forest loses worst where it is nowhere near binding, so the first property
carries the failure.
\item Do not carry a calibrated interval across a device boundary without
saying so, and do not read its width as evidence about it, since split
conformal assumes exchangeable conformity scores and a held-out device makes
that false. What it costs is the same interval with the coverage removed, 90\%
nominal delivering 35\% on an unseen label and 3\% across the size cut at a
width that moves by under 1.5\%, so the distance of item 2 is where to look
for the warning.
\end{enumerate}
...
The disagreement among the models is recorded rather than argued. What each
predicts for SPARC, JT-60SA and ITER is locked in Sec.~S3 of the Supplementary
Material with intervals, a lock date of 31 August 2026 and a content digest
over the input rows; none of the three has a measured H-mode thermal
confinement time at the operating point tabulated, so the table is a record of
predictions rather than a validation. On JT-60SA, inside the database's size
range, all five models agree to within 15\%. On ITER, $1.82\times$ beyond it,
they disagree by a factor of \textbf{8.3}, and the random forest's gap cannot
be closed by tuning, since its prediction cannot exceed the largest training
target, \SI{1.321}{\second}. Among the models that can reach ITER, the
constrained power law of Sec.~\ref{sec:dimensional} predicts
\SI{2.837}{\second} at the $Q=10$ inductive point and the unconstrained refit
\SI{2.858}{\second}, against IPB98(y,2)'s \SI{3.591}{\second}: the two fits
made here sit \textbf{21\%} below the reference law at the operating point it
was used to size. That number is a point estimate from a fit selected on
transfer rather than physics, on a rung chosen after the size-cut scores had
been seen; it points the same way as the newer published laws, which
Sec.~\ref{sec:robustness} finds transferring better here than either fit; and
it is conditional on the $Q=10$ inductive scenario of the 1999
basis~\cite{ipb98} as carried forward in 2007~\cite{doyle07}, since when ITER
has been rebaselined~\cite{loarte}. SPARC, the smaller machine, sits further
from the training data than ITER, at a Mahalanobis distance of 6.9 against
4.7, because its field is 2.1 times the largest in the database: size is not
the only direction in which a next-step device leaves the data behind.

% ishcdb:begin
None of this is settled by one database. Every tokamak result above rests on one
ITPA file, and the fusion evidence now spans two separately assembled
multi-machine databases that share no device, but the second is a stellarator
database, scored on the regressors of its own published law, and it reproduces
the ranking pattern and not the distance diagnostic. The ITER-size-matched
stress test, the interval collapse and the mechanism experiment rest on HDB5
alone.
% ishcdb:end
% ciclop:begin
A further external replication, on the IAEA/IEA CICLOP multi-machine
database~\cite{litaudon24,litaudon26}, was specified and locked in the archived
repository named on the title page before its data were requested; the database had not
been opened to the author by submission, so no result from it is reported, and
the lock stays in force for whatever the data show.
% ciclop:end
% ishcdb:begin
Two replications outside fusion, on Kleiber's
% ishcdb:end
exponent~\cite{kleiber} and on the allometry of the BAAD
compilation~\cite{baad}, reproduce the extrapolation failure in both while the
ranking inversion appears only where the flexible model first wins the
interpolation split; and the nested predictor ladder of the second finds the
failure at every predictor count, including where there is no interpolation
advantage to lose, which is the case with no warning sign at all and the one
these nine features cannot show. Secs.~S4 and~S5 of the Supplementary Material
carry the construction and the caveats.
...
0.165 and ITPA20-IL's 0.158 across the size cut are tied; and the orderings the paper does claim,
...
bounded correction of Sec.~S9 of the Supplementary Material, the constrained fit's 0.183, the
...
The
robustness arms of Sec.~S2 of the Supplementary Material are disjoint in rows but not
...
Sec.~S2 reports the rerun with all 336 shared discharges
removed, which the reversal survives. The locked forecast of
Sec.~S3 is a record, not a validation; only JT-60SA can be checked against
...
these fits have been published: Kardaun~\cite{kardaun} treats the problem
directly, Cordey~\emph{et al.}~\cite{cordey} refit the database under it, and
Verdoolaege~\emph{et al.}~\cite{gls} give a geodesic least-squares regression
built for exactly that noise, and none of the three is attempted here. Refitting
Eq.~\eqref{eq:powerlaw} by orthogonal distance regression instead runs away
along the weak singular direction of the design, moving the major-radius
exponent from 1.58 to 5.40 and more than doubling the in-sample error to 0.454
against 0.181, so these are least-squares exponents, an errors-in-variables fit
would differ, and Sec.~S16 of the Supplementary Material says why no credible
one is offered.
...
The mixed model of Sec.~S10 of the Supplementary Material is scored on eleven
devices, few enough that its one variance component is poorly determined, so
the sweep rather than the fit carries its conclusion; it has a single random
intercept and no random slopes, so what has been ruled out is that a
device-level \emph{level} shift is the missing structure, and Sec.~S10 shows
that a random slope on the features carrying the size dependence is not
identifiable on this database. More broadly, what the results above
...
model with each label weighted equally, reported in Sec.~S11 of the
Supplementary Material, costs the power law more than the forest and takes the
win count to 12 of 13.
...
on all 13 eligible held-out database labels and on all 11 eligible physical
devices once the JET and ASDEX Upgrade variants are collapsed, and to the
published IPB98(y,2) law as
...
Sec.~\ref{sec:meanfunction} then supports long-range saturation, rather than
flexibility alone, as the mechanism across the model families tested; the
three diagnostics on their own cannot separate the two. That ablation runs in
the engineering coordinates, where several
...
Two results temper the diagnosis and neither weakens the practical advice. The
nested predictor ladder of Sec.~S5 finds the extrapolation failure at every
predictor count while the ranking inversion needs the flexible model to win the
interpolation split first, so a field whose scaling law has one predictor faces
the same danger with no warning at all. And separating flexibility from long-range saturation shows the failure
...
Supplementary Material describes.
% ishcdb:begin
The stellarator replication uses the International Stellarator-Heliotron
Confinement Database, assembled under the IEA stellarator-heliotron
collaboration by the teams operating the contributing devices and hosted by IPP
Greifswald and NIFS; the author thanks them for keeping it open.
Neither the ITPA group, the stellarator collaboration nor any contributing
...
Generative AI tools were used in preparing this work, and each is named with
its version and its role.

Anthropic Claude models assisted with developing, debugging and executing
author-specified analysis code in the accompanying repository, and with
drafting and editing passages of this manuscript: Claude Opus 5 (model
\texttt{claude-opus-5}) until mid-September 2026, and Claude Fable 5.1 (model
\texttt{claude-fable-5-1}) from then on, including the external replication
of Sec.~S17 of the Supplementary Material and the revisions made before
submission. One restructuring of the repository, in July 2026, used Claude
Opus 4.8. The repository's commit history records which model assisted with
each change.
...
Sec.~\ref{sec:shift}; and several of the references in Secs.~S14 and~S3 of the
Supplementary Material, which the author then located and checked against their
published records.

None of these systems is an author. The author designed the study, decided which
...
Five third-party datasets underlie this work. None is redistributed here; each
...
\item \textbf{The full DB5.2.3 revision.} Used for the replication of
Sec.~S2 of the Supplementary Material, which scores rows STD5 excludes, and for
the stored-energy, dimensionless and elongation controls of
Sec.~\ref{sec:robustness}, which take \texttt{WTH} and \texttt{KAPPA} from it.
It is a separate
...
% ishcdb:begin
\item \textbf{International Stellarator-Heliotron Confinement Database,
version 26.} The external replication of Sec.~S17 of the
Supplementary Material. Maintained by the IEA stellarator-heliotron
collaboration, described with the ISS04 scaling in Ref.~\cite{iss04}, and
served from an open directory at IPP Greifswald
(\url{https://ishpdb.ipp-hgw.mpg.de/ISS/}), so it is fetched unattended and its
analysis is part of the reproduction run. The analysed file is
\texttt{ISHCDB\_26.txt}, 3311757 bytes, pinned at SHA-256
\texttt{3a9f857b768e\ldots}; the full digest, the frozen
analysis plan and the lock that preceded the download are in the repository.
% ishcdb:end
\end{itemize}
...
\noindent The locked forecast of Sec.~S3 of the Supplementary Material also carries a
...
produced at commit
\begin{center}\scriptsize
\texttt{daa34b611680999bd7817aa8237a79c045fa9804}
\end{center}
...
% ishcdb:begin
The numbers of the stellarator replication are bound the other way round,
because they were written after the rule and not before it: every value the
generated file carries is formatted once by a script, every passage about that
replication is fenced in the source, and the suite fails on any numeral inside a
fence that the script did not produce, so no number in those passages was
typed.
% ishcdb:end
...
\bibitem{iss04} H. Yamada \emph{et al.}, ``Characterization of energy
confinement in net-current free plasmas using the extended International
Stellarator Database,'' \emph{Nucl.\ Fusion} \textbf{45}, 1684 (2005).
\href{https://doi.org/10.1088/0029-5515/45/12/024}{doi:10.1088/0029-5515/45/12/024}
...
saturation and physics constraints in confinement scaling}, version v0.4.15,
Zenodo (2026).
\href{https://doi.org/10.5281/zenodo.22971392}{doi:10.5281/zenodo.22971392}
\end{thebibliography}
