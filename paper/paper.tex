...
published IPB98(y,2) reference. That margin does not survive the question a scaling law exists to answer, and it does not merely shrink.
Under leave-one-label-out the ordering reverses: the forest is worse than that
power law on all 13 held-out labels and all 11 physical devices. It survives nested tuning, threshold and fold sweeps and dropping the
spherical tokamaks; equal label weighting leaves 12 of 13. The loss
power is the target's denominator; retargeting to the stored energy, which
removes the identity alone, leaves it at 9 of 13 and 9 of 11 with the mean gap
intact. The forest's error tracks the held-out label's Mahalanobis distance from the
training distribution ($\rho=+0.85$, against $+0.56$ for a constant baseline)
where the power law's does not. At a cut reproducing ITER's $1.82\times$ jump in major radius the trees score
closer to a constant predictor than to the power law, and 90\% split-conformal
intervals fall to 3\% and 0\% coverage without widening.
...
sample and wins every well-powered cut, and a
...
forest's capped by its training range.
\end{abstract}
...
how a next-step device's performance is now estimated~\cite{doyle07}. The
machine learning in that branch is aimed at a different target from the one
here: neural surrogates are trained to reproduce a physics-based transport model
fast enough to run inside a simulation~\cite{citrin,meneghini}, so what is being
regressed is a solver rather than a database of machines. SPARC's core
performance is projected that way~\cite{sparccore} alongside the empirical
route. The two
...
warning is fixed by the construction rather than by the data it is applied to. Degradation is a
...
where a reversed ranking, and an interval whose width cannot signal that it has
stopped covering, cannot.
...
under grouped cross-validation, and the best cross-validated model \emph{of
those three} loses to a log-linear power law on all 13 held-out database labels
and all 11 physical devices. That qualifier is load-bearing: the best
cross-validated model in this work is the Gaussian process of
Sec.~\ref{sec:gp}, at 0.112, and it does not reverse. Three measurements say why, and
...
That mismatch does not have to be modelled, because it can be measured. STD5 is
an extract; the full DB5.2.3 revision this paper already pins carries the
boundary elongation \texttt{KAPPA} on every analysed row, joined across as in
Sec.~\ref{sec:robustness}. So ITPA20 and ITPA20-IL are scored below on the
elongation they were fitted to, and IPB98(y,2) on the areal elongation it was
fitted to, which is what makes the three comparable at all.

What the substitution would have cost is worth stating, because an earlier
version of this section bounded it with a shape model instead and the model was
wrong. On a parametrised boundary $\kappa/\kappa_a$ depends on the triangularity
alone, which put the ratio between 1.000 and 1.110. The measured ratio has a
median of \textbf{1.083} and a maximum of \textbf{1.341}, with 16\% of rows
above that supposed ceiling and 7\% \emph{below} unity, which no boundary of
that family produces: PBX-M sits at 0.739. Scoring on the delivered column
rather than the measured one moves ITPA20 by up to 0.018 and ITPA20-IL by up to
0.022, which is twenty to thirty times the model's estimate and large enough to
matter to one of the comparisons below. A shape model is not a substitute for a
column that exists.
...
0.188 and 0.194, ITPA20 is below over all rows and at the size cut and level
with it per label; ITPA20-IL is above it at the first two and below it at the
cut, where it is the lowest of the three. Both are below the power law refitted
inside these folds at that cut, which scores 0.278. Against the random forest's 0.938 and the
gradient booster's 1.072 there, ITPA20 is better by factors of 5.3 and 6.1.
...
That control is available, and this paper previously reported that it was not.
STD5 is a 15-column extract carrying \texttt{TAUTH} and \texttt{PLTH} and no
stored energy, which is true and was the wrong file to look in. The full DB5.2.3
revision, already downloaded, pinned by SHA-256 and analysed in
Sec.~\ref{sec:replication}, carries \texttt{WTH} on 13828 of its 14153 rows, and
joining it to STD5 on machine, shot and time recovers a stored energy for
\textbf{6214} of the 6228 analysed rows. \texttt{KAPPAA} is delivered in both
files, so the two copies of it are compared across the join and agree to
$5\times10^{-10}$, which is the check that the rows carried across are the rows
being analysed. On them $W_{\mathrm{th}}/(\tau_{\mathrm{th}}P)$ has a median of
1.000000 and quartiles of 0.981 and 1.000.

Retargeted, the direction and most of the magnitude survive; the unanimity does
not. The forest still wins cross-validation, 0.121 against the power law's
...
cannot'' is therefore available, and this is the measurement that rules it out.
The specification is in \texttt{analysis\_stored\_energy.py} and
...
Sec.~\ref{sec:dimensional} at 0.183 and the analytic law's 0.194. What has been ruled out is
...
Verdoolaege~\emph{et al.}~\cite{gls} give a regression for confinement data
...
exactly and a tree cannot. Two controls in Sec.~\ref{sec:robustness} separate
what that identity carries from what it does not. Regressing the stored energy
instead, which leaves the loss power in the feature set as an ordinary predictor
and removes only the identity, takes the count to 9 of 13 and 9 of 11 and leaves
the magnitude alone: the mean paired gap is $+0.220$ by label and $+0.348$ by
device, against $+0.251$ and $+0.315$, and across the size cut the forest scores
1.081 against the power law's 0.289. Deleting the loss power outright, which
also deletes a real predictor, is the harsher experiment and costs more, at 10
of 13 and a mean gap of $+0.051$. So the identity carries the unanimity and the
exact tests, which fall below significance under either control, and it does not
carry the effect. Three separate
...
\textbf{Eighty-five} of the numbers printed above, and \textbf{one hundred and
nine} across this paper and its Supplementary Material, are tied by test to the
field of the generated file they came from, in both directions: if an analysis is rerun and its output moves, the
