...
linear component keeps trending. Extrapolation
failure tracks long-range saturation rather than flexibility alone. For ITER the
...
against leave-one-device-out and labelled as such. The 30-row threshold governs only which machines are scored, not which
...
removes that non-independence, the forest's correlation weakens slightly to
$+0.77$ ($p=0.005$) and the power law's is unchanged at $-0.06$ ($p=0.85$),
so the contrast the section rests on survives the collapse. The booster's falls
...
precisely. Call it the independent-device-sign null: each unit is assigned to
either model independently with equal probability. Under it, losing 13 of 13 has
...
schemes, on a held-out label and across the ITER-size-matched cut.}
\label{tab:shift}
\begin{tabular}{lrrrrrr}
\toprule
...
with the whole
database as the training set. That scheme was chosen after comparing the three
in Table~\ref{tab:shift}, so these intervals are exploratory rather than
prospectively selected, in the same sense as the constraint rung of
Sec.~\ref{sec:dimensional}. For a given model, each eligible machine $k$ is
...
Anthropic Claude Opus 5 (model \texttt{claude-opus-5}) was used to assist with
developing, debugging and executing author-specified analysis code in the
accompanying repository, and to draft and edit passages of this manuscript. It
did not choose what to analyse: every experiment, split and comparison reported
here was specified by the author. OpenAI GPT-5.6 Sol was used
...
Allometry Database for woody plants}, release v1.0.1 [dataset] (2015).
