...
\begin{abstract}
\noindent
Confinement scalings are fitted across existing tokamaks to size machines not
yet built, so validation must measure performance on an unseen device. On the
ITPA Global H-mode Confinement Database (HDB5, STD5; 6228 slices, 18 labels),
the conventional cross-validation grouped by discharge scores a random forest
...
\textbf{13 of 13} labels. Regressing the stored energy instead, or in
dimensionless coordinates carrying far less device identity, leaves the forest worse on
9 of 13 and 11 of 13 labels. The intervals fail more sharply. At a cut inside
...
moving under 1.5\%. The width cannot respond: a split-conformal half-width is
a rank statistic of the calibration residuals, so the interval carries no
warning that it has stopped covering. The warning does exist: the forest's
...
one ITPA file. Two replications outside fusion, on Kleiber's
...
produced at commit
\begin{center}\scriptsize
\texttt{0630b3574c9086d98632802786ad2a42b2e01717}
\end{center}
