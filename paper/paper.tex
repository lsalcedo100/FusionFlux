...
prediction~\cite{katesharbeck}, predictors trained and compared across two
devices rather than one~\cite{rea}, transfer of a disruption predictor to a
device that supplied almost none of its training data~\cite{zhu}, and real-time
magnetic control~\cite{degrave}. The disruption branch reached the
cross-machine question first, and reached it with a target that can be scored
shot by shot on the new machine, which is what a confinement scaling law for a
device that does not yet exist cannot have. Confinement scaling itself has been revisited
...
figure here regenerates from the published OSF deposit at the single commit
named in Data availability, which also gives the pinning and the test binding
that hold the prose to the generated files. Nothing below asks to be believed.
...
Selecting on the cross-validated score is what the convention prescribes, and
applied to the three primary models here it picks the one that extrapolates
worst, so a practitioner comparing that set is not left with an open question
about extrapolation but led to the worst available answer, the more confidently
the more carefully the convention is followed. The scope of that is the
candidate set rather than the score: Sec.~\ref{sec:gp} fits a model with
comparable flexibility and a different asymptote, at once the best
cross-validated model in this work and one that extrapolates, so widening the
set repairs the rule rather than replacing it.
...
the three primary fitted models reverses against their ranking under grouped
cross-validation, and the best cross-validated model \emph{of those three} loses
to a log-linear power law on all 13 held-out database labels and all 11 physical
devices. That qualifier is load-bearing: the best
