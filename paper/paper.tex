...
Archived at \href{https://doi.org/10.5281/zenodo.22562235}{doi:10.5281/zenodo.22562235}
(v0.4.2; the DOI for all versions is
...
Two repairs improve extrapolation, both supplying long-range structure.
Constraining the exponents to a Connor--Taylor similarity surface helps
prospectively: the Kadomtsev rung, fixed by the literature in 1975 and
independent of anything measured here, costs nothing in sample and beats the
unconstrained fit at all 15 cuts of a size sweep. The collisionless rung scores
lowest of any fit at the size-matched cut but was selected post hoc, so it is
the best of three surfaces tested rather than a prediction. The second repair is
a Gaussian process whose linear component keeps trending. Across the model families tested,
extrapolation failure tracks long-range saturation rather than flexibility
alone. For ITER the
IPB98(y,2) and random-forest point predictions differ by a factor of 8.3.
\end{abstract}
...
completely once the held-out unit becomes a whole device. We do
...
imposed as linear equality constraints on the exponents, where the rung fixed by
the literature in advance beats the unconstrained fit at all 15 cuts of a size
sweep at no cost in sample, and the rung selected after those scores were seen
gives the lowest observed error of any fit tested here at that cut; (v) a Gaussian-process experiment
...
the three on a label it has not seen. It is not the best model in the paper:
the Gaussian process of Sec.~\ref{sec:gp} scores 0.112. Both columns compare the same model
...
confinement time anywhere in the training fold, and its largest held-out target is
...
the split: large machines are few, and their scarcity is the same fact that
makes the extrapolation hard in the first place. The
leave-one-label-out result of Sec.~\ref{sec:reversal}, which holds on all 13
labels and all 11 devices, is the one that rests on many machines; this
one rests on a large jump. Dropping the spherical tokamaks moves the
...
labels and by the widest margin on JT-60U, the most distant (0.440 to 0.281).
...
jump to ITER, the bounded hybrid equals the power law at $\lambda=0$, where it
is that model, and beats it at all eight non-zero damping settings swept there,
with the mechanism measurable.
...
collisionless rung is the best of the three at the size cut and was chosen after
those scores were seen. Both results are reported below, in that order, and they
...
A constraint the data satisfies on average is not thereby idle: satisfying a
constraint on average is not the same as being unable to violate it, and what
the constraint buys is refusal to wander along that direction when the training
half is small or shifted. The underpowered rungs show the mechanism directly. At
six training machines the unconstrained fit scores 0.971 and the Kadomtsev fit
0.274. Nothing about that comparison was selected after the fact, and it is the
result in this section a reader should weigh most heavily.

The collisionless rung is the stronger assumption and the better score, and it
was chosen post hoc. Two things can still be said for it before any score is
consulted, and they are worth separating from the selection. It is a physics
judgement about the machine being extrapolated \emph{to} rather than a
hyperparameter: one adopts it because one expects the target plasma to be
collisionless, and ITER is expected to sit at lower normalised collisionality
than the devices in this database rather than higher, which points at this rung
in advance and for a reason external to the data. And it has no
within-surface quantity to tune, so there is nothing for a validation split to
fit even in principle. Neither of those makes the result prospective, and the
disclosure stands as written.
...
no within-surface hyperparameter to tune, though the surface itself was
selected post hoc from the three tested. Figure~\ref{fig:dimensional} shows the trade between what each
...
Sec.~\ref{sec:conformal}. And in the distance-scaled arm the scale function
...
label is the collisionless power law of Sec.~\ref{sec:dimensional}. Counting label--model pairs
...
Table~\ref{tab:forecast} as correspondingly optimistic in width. For a given model, each eligible label $k$ is
...
Sec.~\ref{sec:shift} measures directly.
...
which is why its worst label is 4.601. No model to this point was flexible,
...
The consequence is a reporting convention rather than a modelling one, and it is
...
\item \textbf{Report leave-one-device-out beside cross-validation.} It costs one
refit per device, on the same rows and the same features, and the two numbers
print together. Where they agree the interpolation number is informative. Where
they disagree, as here by a factor of 3.6, the disagreement is the result, and
the interpolation number should not be quoted on its own.
\item \textbf{Report the distance of the target operating point from the
training distribution.} This needs no held-out device. The Mahalanobis distance
of a label's mean log-feature vector tracked per-label error at $\rho=+0.85$ for
the random forest and at $\rho=-0.06$ for the power law, so it separates the
models that will degrade from the ones that will not, in advance, and without
waiting for the machine to exist.
\item \textbf{State whether the model's predictions can leave the range of its
training targets.} A random forest provably cannot, since every prediction is an
average of training targets. A model that cannot exceed the largest confinement
time it has seen cannot project a machine expected to exceed it, and that is a
property of the estimator, checkable before anything is fitted.
\item \textbf{Do not carry a calibrated interval across a device boundary
without saying so.} Split conformal assumes exchangeable conformity scores, and
across a held-out device that assumption is false. What it costs is not a wider
interval but the same interval with the coverage removed: 90\% nominal
delivering 35\% on an unseen label and 3\% across the size cut, at a width that
moves by under 1.5\%.
\end{enumerate}

\noindent None of the four requires a new model, a new dataset, or a new method.
The first costs one refit per device; the other three cost nothing and are
...
rung selected after those scores had been seen costs 0.001 and was the
best-scoring option at the size cut.
...
law with each training label weighted equally does not help, moving its
...
separates this database from ITER, the two tree ensembles that lead the
conventional cross-validation comparison land closer to a constant predictor
than to the power law, and their nominal 90\% intervals cover 3\% and 0\% of
the held-out rows at unchanged width. In these tests the
...
on its residuals improves it along the size-extrapolation direction tested here,
...
it is a dimensional constraint on the existing power law, and the two rungs
tested carry different weight. The Kadomtsev rung was fixed by the literature in
1975 and enforced when IPB98(y,2) was fitted, so imposing it here selected
nothing: it costs nothing in sample and beats the unconstrained fit at all 15
cuts of the size sweep. The collisionless rung gave the lowest observed error of
any fit tested here at the ITER-size-matched cut, cost 0.001 in sample, and has
no within-surface hyperparameter to tune, but it was selected after the held-out
scores at this cut had been seen, as Sec.~\ref{sec:dimensional} states, so it is
the lowest error among the surfaces examined rather than a prediction that this
surface is the right one. Supplying
...
The inversion is therefore robust to the validation, aggregation and
row-selection choices tested here, rather than an artifact of a single
...
produced at commit
\begin{center}\scriptsize
\texttt{27a5c0a66fe45491080d572f48452ef5bbb75620}
\end{center}
...
cross-validation loses on a new machine}, version v0.4.2, Zenodo (2026).
\href{https://doi.org/10.5281/zenodo.22562235}{doi:10.5281/zenodo.22562235}
\end{thebibliography}
