...
Under
leave-one-label-out the ordering of the three primary models reverses: the
forest is worse than that same power law on all 13 held-out
database labels and all 11 physical devices they reduce to. It survives nested tuning, threshold and fold-assignment sweeps, and dropping
the spherical tokamaks; equal label weighting leaves it at 12 of 13. The forest's error tracks the Mahalanobis
distance of the held-out label from the training distribution ($\rho=+0.85$)
where the power law's does not ($\rho=-0.06$), and it cannot predict outside
the range of its training targets. At a cut reproducing ITER's $1.82\times$ jump in major radius, the tree
ensembles score closer to a constant predictor than to the power law, and 90\% split-conformal intervals fall to 3\% and 0\% coverage at
unchanged width.
Two repairs improve extrapolation, both supplying long-range structure.
Connor--Taylor constraints on the exponents help prospectively: the
Kadomtsev rung, fixed in 1975, is free in sample and beats the unconstrained
fit at all 8 well-powered cuts of a size sweep. The collisionless rung scores lowest at the size-matched cut but was selected
post hoc. The second repair is a Gaussian process whose linear component keeps
trending. Across the families tested, extrapolation failure tracks long-range
saturation rather than flexibility. For ITER the IPB98(y,2) and forest predictions differ by 8.3$\times$.
\end{abstract}
...
and report that cross-machine transfer is harder, so it is worth saying plainly
what is different here rather than leaving it to the Discussion. Their claim,
and the one that literature generally supports, is that accuracy
\emph{degrades} out of distribution. This paper makes two claims that do not
follow from that one and have different consequences. The first is that the
\emph{ranking} reverses: the model a practitioner would select on the
conventional split is not merely worse on an unseen device but worse than the
baseline it was selected over, so the selection procedure returns the wrong
model rather than an optimistic score for the right one. The second is that the
calibrated intervals fail \emph{at unchanged width}: 90\% intervals covering 3\%
and 0\% of held-out rows, having become confidently wrong rather than usefully
vague, which is the failure mode a design study cannot detect from the interval
itself. Degradation is a matter of degree and can be absorbed by a margin.
...
held out in four steps, a discharge, then a database label, then a physical
device, then an entire size range, and score the same models under each. Under
the second step the ranking of the three primary fitted models is the exact
reverse of their ranking under grouped cross-validation, and the best
cross-validated model loses to a log-linear power law on all 13 held-out
database labels and all 11 physical devices. Three measurements say why, and
they cover extrapolation distance, the training-target bound of a random
forest, and what polynomial flexibility costs in the tail. At the fourth step,
...
second confinement regime, and on two scaling laws from biology.
...
polynomial and Gaussian-process rungs, are written out in full in Sec.~S6 of the
...
AUG-W, whose unweighted margin was the narrowest of the thirteen at $+0.012$ and
...
across size in general. The claim the paper rests on is the leave-one-device-out
result of Sec.~\ref{sec:reversal}, which holds out 11 physical devices one at a
time and does not depend on where a size boundary is drawn.
...
\caption{\textbf{The interval collapse.} Left: empirical coverage of nominal
...
a whole label and then a size range. Right: per-label coverage against the
...
those scores were seen. The prospective one comes first below.
...
analysis; the collisionless row is the best of the three and was selected after
these scores were seen.}
...
was chosen post hoc. Something can still be said for it that does not depend on
the score. It is a physics judgement about the machine being extrapolated
...
to exist.
\item Say whether the model's predictions can leave the range of its training
targets. A random forest provably cannot, since every prediction is an average
of training targets, and a model that cannot exceed the largest confinement time
it has seen cannot project a machine expected to exceed it. That is a property
of the estimator, checkable before anything is fitted.
...
tokamaks. The same audit run on two scaling laws outside fusion, Kleiber's law
for mammalian metabolic rate~\cite{kleiber} and a predictor-count ladder on
woody plants~\cite{baad,wbe}, reproduces the extrapolation failure in both and
the ranking inversion only where the flexible model first wins the
interpolation split. Both are reported in the Supplementary Material; they
support the reading of the fusion result rather than carrying it.
...
characterisation of how the result behaves under reweighting in general. The Gaussian-process intervals of Sec.~\ref{sec:gp} are
...
well, though that law is a historical reference fitted to the DB2.8 predecessor
of this database and was not refitted within these folds. Three separate
diagnostics point at that failure and none of them is a tuning fix; the
mechanism they converge on is long-range saturation. At the size extrapolation
...
any fit tested here at the ITER-size-matched cut, but was selected after those
scores had been seen, so it is the lowest error among the surfaces examined
rather than a prediction that this surface is the right one.

Two results temper the diagnosis and neither weakens the practical advice. In
the nested predictor ladder the extrapolation failure persists at every
...
produced at commit
\begin{center}\scriptsize
\texttt{ba6d4b5367b232320af72e578acfbb8a43d51237}
\end{center}
