...
Archived at \href{https://doi.org/10.5281/zenodo.22431039}{doi:10.5281/zenodo.22431039}
(v0.3.0; the DOI for all versions is
...
next-step tokamak has to reach, and they are fitted across existing devices. On
the ITPA global H-mode database (HDB5, standard analysis set STD5; 6228 slices
from 4471 discharges across 18 database device/wall labels, corresponding to 16
physical devices), under cross-validation grouped by discharge, a random forest
reduces log error by 29\% against a power-law baseline refitted within the same
folds and 36\% against the published IPB98(y,2) reference. That margin does not
survive the question a scaling law exists to answer. Under leave-one-machine-out
the ordering of the three primary fitted models reverses exactly: the forest is
worse than that same power law on all 13 held-out database labels and all 11
physical devices scored (sign test $p=9.8\times10^{-4}$). The reversal survives a
hyperparameter search nested inside every training fold, a sweep of the row
threshold that decides which labels are scored, and repeated cross-validation
over shuffled fold assignments. The forest's error tracks the Mahalanobis
distance of the held-out machine's mean log-feature vector from the training
distribution ($\rho=+0.85$) where the power law's does not ($\rho=-0.06$), and its
predictions cannot exceed the range of its training targets. At a size-ordered
cut reproducing ITER's $1.82\times$ jump in major radius, the tree ensembles
score closer to a constant predictor than to the power law, and split-conformal
intervals calibrated to 90\% fall to 3\% row-level coverage for the forest and
0\% for the booster, at unchanged width. Two exploratory repairs improve
extrapolation, both adding long-range structure rather than flexibility: a
Connor--Taylor constraint surface selected post hoc from the size-cut scores,
and a Gaussian process whose linear component keeps trending. Across the
families tested here, extrapolation failure tracks long-range saturation rather
...
like-for-like baseline, and the reversal below is stated against it. The
repository
...
leave-one-machine-out the ranking of the three primary fitted models is the
...
analysis set STD5 v5.2.3~\cite{hdb5}, obtained from OSF. The delivered file
...
regimes, so for most folds it carries information about the held-out machine;
...
shuffled partitions. (ii) \emph{Leave-one-machine-out} (LOMO in the tables):
each database label with at least 30 rows is held out in turn and the model is
trained on every other machine in the database. Two units are used and named
separately where the difference matters, since the 18 labels are not 18 physical
devices: \emph{leave-one-label-out} holds out one of the 13 eligible database
device/wall labels, and \emph{leave-one-device-out} holds out one of the 11
physical devices those labels reduce to when JET and JET-ILW are merged, as are
the two ASDEX Upgrade entries. Unqualified, LOMO means leave-one-label-out over
the 13. The 30-row threshold governs only which machines are scored, not which
...
round number giving a per-machine log-RMSE over enough rows to be worth
...
(LOMO is leave-one-label-out over the 13 eligible labels). The two columns
...
117\% worse in the LOMO column. The analytic IPB98(y,2) row is the published
...
last of them on a held-out machine.
...
\centering
\small
\caption{Per-machine log-RMSE under leave-one-machine-out, ordered by Mahalanobis
distance of the held-out machine's mean log-feature vector from the training
...
Table~\ref{tab:threshold} scores leave-one-machine-out at 10, 20, 30 and 50
...
below the threshold in the first place, and a per-machine log-RMSE over 11 rows
...
\centering
\small
\caption{Leave-one-machine-out at four eligibility thresholds. Only the set of
labels scored changes across rows; the training folds and the models are
identical throughout. The 30-row row is the one reported elsewhere in this
paper. $p$ is the exact two-sided sign test on the win count.}
\label{tab:threshold}
...
best on a held-out machine is $\lambda=0$; nothing in the CV signal points at
...
pooled over rows; under leave-one-machine-out each held-out machine is
...
ITER to 2.9\%. None of the three has a published H-mode thermal confinement
...
scenario). Intervals are distance-scaled machine-level conformal intervals, not
...
the distance-scaled machine-level scheme of Sec.~\ref{sec:shift}, the
best-calibrated scheme out of distribution in this work, applied with the whole
database as the training set. For a given model, each eligible machine $k$ is
...
in a later campaign; we deliberately do not put a date on that here, because the
...
Take the control first: with a constant mean, Eq.~\eqref{eq:meanfunction}
...
leave-one-machine-out on the same rows and the same features, at a cost of one
...
dimensionality for the flexible model to win the interpolation comparison
first, which the ladder establishes and which means a field whose scaling law
has one or two predictors faces the same extrapolation hazard with no warning
...
Anthropic Claude Opus 5 (model \texttt{claude-opus-5}) was used to help develop
the analysis code in the accompanying repository, to run parts of the analysis,
and to draft and edit passages of this manuscript. OpenAI GPT-5.6 Sol was used
to review and critique complete drafts against journal policy and for
scientific and historical accuracy; corrections and wording it proposed were in
places adopted, including the corrected description of how IPB98(y,2) was
fitted in Sec.~\ref{sec:reversal} and Sec.~\ref{sec:dimensional}. No generative
AI system contributed the research design, selected the experiments, or is an
author, and no text was incorporated without the author reading and verifying
it. The author designed the study, verified every reported number against the
generated artifacts, checked every reference against its published record, and
takes full responsibility for the content, the conclusions and any error in
them.
...
cross-validation loses on a new machine}, version v0.3.0, Zenodo (2026).
\href{https://doi.org/10.5281/zenodo.22431039}{doi:10.5281/zenodo.22431039}
\end{thebibliography}
