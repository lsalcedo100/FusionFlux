...
individually: TFTR
contributes 2 rows, far below the 30 required to report a label on its own,
...
when the test rows come from labels the model has seen. It fails by construction when the test
rows come from a label the model never saw; that exchangeability is the
...
carries a finite-sample guarantee: the calibration units are all smaller than
...
refit per unit, and the two numbers can be printed together. Where they
