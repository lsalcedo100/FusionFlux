...
(v0.4.14; the DOI for all versions is
\href{https://doi.org/10.5281/zenodo.22215142}{10.5281/zenodo.22215142})
\end{center}

\begin{abstract}
\noindent
Confinement scalings fitted across existing tokamaks size machines not yet
built, so validation must measure performance on an unseen device. On the
ITPA Global H-mode Confinement Database (HDB5, STD5; 6228 slices, 18 labels),
conventional cross-validation grouped by discharge scores a random forest
...
the order of the three primary models: the two tree ensembles that win
cross-validation extrapolate worst, and the forest loses to that power law on
% ishcdb:begin
\textbf{13 of 13} labels, and on \textbf{7 of 7} stellarators in a second
database locked before its data were seen.
% ishcdb:end
...
a rank statistic of the calibration residuals, so the interval carries no
warning that it has stopped covering. The warning exists: the forest's
error tracks the held-out label's Mahalanobis distance from the training data
at $\rho=+0.85$, against $-0.06$ for the power law. The fault is the candidate
set, not cross-validation: a linear-plus-RBF Gaussian process, the best
cross-validated model, keeps trending outside the data and does not reverse.
At the size cut, that process and Connor--Taylor similarity constraints on the
exponents, whose Kadomtsev rung costs nothing in sample, both improve
extrapolation. Varying only the long-range mean, with the nonlinear part
fixed, identifies the mechanism as saturation rather than flexibility.
\end{abstract}
...
reports it, and the inversion reproduces there on every held-out device.
% ishcdb:end
...
saturation and physics constraints in confinement scaling}, version v0.4.14,
Zenodo (2026).
