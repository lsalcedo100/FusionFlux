...
\begin{abstract}
\noindent
Confinement scaling laws set the size a next-step tokamak has to reach. On the International Tokamak Physics Activity (ITPA) Global H-mode Confinement
Database (HDB5, standard analysis set STD5; 6228 slices, 18 device/wall labels),
...
published IPB98(y,2) reference. That margin
does not survive the question a scaling law exists to answer. Under
leave-one-label-out the ordering of the three primary models reverses
exactly: the forest is worse than that same power law on all 13 held-out
database labels and all 11 physical devices they reduce to. It survives a nested hyperparameter search, a row-count threshold sweep and
shuffled fold assignments. The forest's error tracks the Mahalanobis
distance of the held-out label from the training distribution ($\rho=+0.85$)
where the power law's does not ($\rho=-0.06$), and its predictions cannot
exceed the range of its training targets. At a cut reproducing ITER's $1.82\times$ jump in major
...
coverage at unchanged width.
Two repairs improve extrapolation, both supplying long-range structure.
Connor--Taylor similarity constraints on the exponents help prospectively: the
Kadomtsev rung, fixed by the literature in 1975, costs nothing in sample and
beats the unconstrained fit at all 15 cuts of a size sweep. The collisionless
rung scores lowest at the size-matched cut but was selected post hoc. The second
is a Gaussian process whose linear component keeps trending. Across the
families tested, extrapolation failure tracks long-range saturation, not
flexibility alone. For ITER the IPB98(y,2) and random-forest predictions differ
by a factor of 8.3.
\end{abstract}
