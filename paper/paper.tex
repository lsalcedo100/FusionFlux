...
dimensionless coordinates that identify no device, leaves the forest worse on
...
Sec.~\ref{sec:rank}, which adds IPB98(y,2)'s own prediction to them and is
reported there as a control, and the controls of Sec.~\ref{sec:robustness}
...
What this arm adds is not the coordinates but the use they are put to: a
control on the device-label pathway, scored under the held-out-device split
against the same models the engineering features were scored on. In
...
\subsection{A power law with a bounded correction}
\label{sec:hybrid}

This is a supporting result and it is reported at that weight: it is built and
scored in Sec.~S9 of the Supplementary Material, and what follows is the
summary. The diagnosis in Sec.~\ref{sec:mechanisms} suggests the repair: fit the power law, learn a correction
...
Two limits keep it from being a recommendation. Cross-validation selects the most heavily
