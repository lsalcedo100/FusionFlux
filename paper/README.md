...
carries seventeen sections. S1 to S3 are fusion analyses whose headline numbers stay
...
Limitations paragraph). S17 was appended under the same rule when the external
replication on the stellarator-heliotron confinement database ran: its analysis
plan was locked and archived before the file was downloaded
(`../docs/ishcdb-replication-lock.md`), and every number in it is generated by
`tools/ishcdb_tables.py` rather than typed. The former Secs. 5.1, 9 and 10 were folded into S9, S2
...
3. Push a tag and publish a GitHub release, then Zenodo archives it and mints
   ...
   Then **confirm the record exists** before doing anything that cites it.
   `v0.4.14` was published on GitHub on 25 September 2026 and Zenodo never
   archived it: the concept DOI went on resolving to `v0.4.13`, and
   `docs/releases.json` records `v0.4.14` with no DOI for that reason. A
   missing record is a failure to chase on Zenodo's GitHub settings page,
   where the release event and its error are shown, not a delay to wait out.
