...
A tokamak holds a plasma hot enough to fuse inside a magnetic field, and the plasma leaks its heat back out. How long it holds that heat, the **energy confinement time**, is a principal input to how large the machine has to be before fusion produces more energy than it consumes. Empirical scaling remains a principal tool for projecting it, so the field predicts it with a power law fitted across the machines already built, and ITER is being sized against that prediction.
...
> A random forest beats a power law refitted inside the same folds by **29%** under the standard way these models are validated, and the published IPB98(y,2) law by 36%. Under a split that holds out an entire tokamak, it is **worse than that same power law on 13 of 13 held-out database labels, and 11 of 11 physical devices**. The standard validation does not merely overstate the gain, it reverses the ranking, and the reason is measurable rather than a matter of tuning.

Everything here is measured on real experimental data: the ITPA Global H-mode Confinement Database, standard analysis set STD5, 6228 quasi-stationary time slices from 4471 discharges across 18 database device/wall labels, corresponding to 16 physical devices. (The database paper describes STD5 as 7537 points; the file the OSF deposit delivers under that name has 6228, and it is analysed as delivered, with nothing removed here.) Every model is scored against the analytic IPB98(y,2) scaling law, the published physics baseline, rather than against a mean predictor. No synthetic data enters any reported result. The dataset is fetched from OSF rather than committed and is **pinned by SHA-256**, verified on every load, so each number below is tied to a specific set of bytes rather than to whatever the host is currently serving.

Then you ask whether that is a fact about this one database, and it is not: the same reversal reproduces on 5358 rows the standard set does not contain, and again in a different confinement regime scored against a different published law. And you ask whether anything repairs it. **A Connor-Taylor dimensional constraint does**, and it costs nothing in sample.
...
Under cross-validation grouped by discharge, a random forest scores 0.128 in log-RMSE against 0.181 for a power law refitted inside the same folds on the same rows and features, a margin of **29%**, and against 0.199 for the published IPB98(y,2) law, a margin of 36%. The fold-fitted refit is the like-for-like comparator and the one the headline above quotes; IPB98(y,2) was derived in 1998 from DB2.8, a predecessor of this database that overlaps it heavily in devices and regimes, so it is a historical reference rather than a fold-specific competitor. But grouped CV holds out *shots*, so every machine in the held-out fold is also in the training fold. Hold out an entire tokamak instead, train on every other machine in the database and predict the held-out one, and **the ranking of the three fitted models reverses exactly**:
...
**Neither the row threshold nor the fold assignment carries it.** Leave-one-machine-out scores only the labels with at least 30 rows, and grouped cross-validation uses one deterministic assignment of discharges to folds. Both choices are swept in `analysis_referee_robustness.py`. The power law wins every scored label at thresholds of 20, 30 and 50 rows, and 14 of 15 at a threshold of 10, where the single exception is TCV at 11 rows. Repeating the cross-validation over ten shuffled fold assignments gives a mean forest advantage of 27.8% (range 27.0% to 28.5%, forest ahead in all ten) against 29.5% for the deterministic partition, so the reported interpolation margin is if anything mildly generous to the forest.
...
|---|---|---|---|---|
| IPB98(y,2), analytic (historical reference) | 0.199 | 0.188 | **0.194** | 1.00 |
...
| IPB98(y,2), analytic (historical reference) | - | 0.199 | 0.188 | 0.194 |
| hybrid, above | - | 0.151 | 0.246 | 0.206 |
| power law, Kadomtsev | 0.1808 | 0.181 | 0.211 | 0.254 |
| ridge, log-linear | 0.1808 | 0.181 | 0.214 | 0.278 |

**0.183 is the best score any fold-fitted model in this repository reaches at the ITER-size-matched cut.** It beats the hybrid above, and it beats the analytic law, which was itself fitted under a Kadomtsev constraint on a predecessor database. That rung was selected after the size-cut scores had been seen, so it is the lowest error among the surfaces examined rather than a prospective prediction that this surface is the right one. It has no hyperparameter and nothing to tune: the only difference from the ridge row is the constraint.
...
|---|---|---|---|
| IPB98(y,2), analytic (historical reference) | 90% | 89% | 88% |
...
- `ishcdb.py` and `analysis_ishcdb.py` are a second locked replication, on the International Stellarator-Heliotron Confinement Database, which is public and shares no device with HDB5. Its rules are in `docs/ishcdb-replication-lock.md`, fixed from the database's documentation before the file was downloaded. The run of record is `results/ishcdb.json`, and Sec. S17 of the supplement reports it: under the predeclared rule the ranking inversion reproduces on every held-out device, and the distance diagnostic does not.
