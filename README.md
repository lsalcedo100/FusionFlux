...
`tests/test_compare_results.py` covers the comparator behind that gate, since a comparator that quietly passed everything would leave the workflow green while guarding nothing. `tests/test_release_metadata.py` does the same for `.zenodo.json`, which nothing else in the repository consumes and which Zenodo turns into a permanent record: it checks the fields Zenodo requires, that the author and licence agree with `CITATION.cff` and `LICENSE`, that the description still says the HDB5 data is third-party and not redistributed, and that no second copy of the file exists to drift from the real one.
