...
- `ishcdb.py` and `analysis_ishcdb.py` are a second locked replication, on the International Stellarator-Heliotron Confinement Database, which is public and shares no device with HDB5. Its rules are in `docs/ishcdb-replication-lock.md`, fixed from the database's documentation before the file was downloaded.
